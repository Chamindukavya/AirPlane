...
export default function Component(props: { routeId: number }) {
  const [chartData3, setChartData] = useState<any[]>([]);
  const [nextFlight, setNextFlight] = useState<string>("");
  const [activeMonth, setActiveMonth] = useState<string | undefined>(undefined);

  useEffect(() => {
    if (chartData3.length > 0 && !activeMonth) {
      setActiveMonth(chartData3[0].month); // Set active month once data is available
    }
  }, [chartData3, activeMonth]); // Run this effect when passengerCount or activeMonth changes
  ...
  async function fetchData() {
    ...
    const chartData = [
      {
        browser: "Under18",
        visitors: data.under18,
        fill: "var(--color-chrome)",
      },
      {
        browser: "Above18",
        visitors: data.over18,
